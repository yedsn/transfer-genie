...
设置界面 SHALL 提供是否在语音录制时录制电脑内部声音的配置；默认 SHALL 关闭该选项。该选项 SHALL 提供独立于麦克风的电脑内部声音输入设备选择框，并明确说明当前实现需要 macOS 原生采集支持或虚拟音频输入设备。用户安装 BlackHole、Loopback 等虚拟声卡后 SHALL 能在电脑内部声音设备选择框中选择该设备，并与麦克风同时录制。保存、导入和导出配置 SHALL 包含该字段并向后兼容旧版本。
...
- **THEN** 设置界面提示用户通过电脑内部声音设备选择框选择 BlackHole、Loopback 等虚拟音频设备来录制系统声音
