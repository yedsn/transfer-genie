...
### Requirement: Speech Cue Sound Settings
设置界面 SHALL 提供语音录制开始/结束提示音配置；默认 SHALL 启用系统提示音，用户 SHALL 能选择内置提示音类型、试听当前提示音或关闭提示音。保存、导入和导出配置 SHALL 包含该字段并向后兼容旧版本。
...
#### Scenario: 关闭语音提示音
- **WHEN** 用户在设置中关闭语音录制提示音并保存
- **THEN** 后续开始和结束语音录制时不播放提示音

#### Scenario: 修改语音提示音
- **WHEN** 用户选择另一个内置提示音类型并保存
- **THEN** 后续开始和结束语音录制时使用所选提示音类型

#### Scenario: 试听语音提示音
- **WHEN** 用户在设置中点击试听
- **THEN** 系统使用当前未保存的提示音设置立即播放一次提示音预览
- **AND** 不启动录音或修改已保存设置

### Requirement: System Audio Capture Settings
设置界面 SHALL 提供是否在语音录制时录制电脑内部声音的配置；默认 SHALL 关闭该选项。该选项 SHALL 提供独立于麦克风的电脑内部声音输入设备选择框，并明确说明当前实现需要 macOS 原生采集支持或虚拟音频输入设备。用户安装 BlackHole、Loopback 等虚拟声卡后 SHALL 能在电脑内部声音设备选择框中选择该设备；应用 SHALL 提示用户先在 macOS 音频 MIDI 设置或系统声音输出中将电脑声音路由到该虚拟设备，再与麦克风同时录制。保存、导入和导出配置 SHALL 包含该字段并向后兼容旧版本。

#### Scenario: 默认不录制电脑声音
- **WHEN** 用户首次打开设置或加载旧版本配置
- **THEN** 语音录制只采集麦克风声音
- **AND** 电脑内部声音录制选项处于关闭状态

#### Scenario: 启用电脑声音录制
- **WHEN** 用户开启电脑内部声音录制并保存设置
- **THEN** 设置界面提示用户通过电脑内部声音设备选择框选择 BlackHole、Loopback 等虚拟音频设备，并先在 macOS 中把电脑声音路由到该设备
- **AND** 后续语音录制 SHALL 同时使用麦克风设备和电脑内部声音设备，不弹出屏幕或窗口选择器

#### Scenario: 选择虚拟输入设备录制电脑声音
- **WHEN** 用户安装虚拟音频设备并在电脑内部声音输入设备中选择该设备
- **THEN** 语音录制将麦克风输入和该虚拟输入设备混音为同一段音频
- **AND** 应用不调用屏幕共享或显示捕获接口
